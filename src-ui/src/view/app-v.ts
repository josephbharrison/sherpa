...
export class AppView extends BaseHTMLElement {
    ...
    async onRouteChange() {
        const { system_id } = router.get_current();
        if (system_id != null) {
            const system = await systemFmc.get(system_id);
            const systemEl = elem('system-v', { $: { system } });
            this.#mainEl.replaceChildren(systemEl);
        } else {
            this.#mainEl.textContent = 'Welcome select system';
        }
    }
    ...
    init() {
        // Will be called by BaseHTMLElement once on first connectedCallback
        ...
        if (h1) {
            if (h1.firstElementChild == null) {
                const text = h1.textContent?.split(/[-_ ](.+)/) ?? [
                    'NO',
                    'NAME',
                ];
                h1.replaceChildren(
                    html`<span>${text[0]}</span
                        ><span class="prime">${text[1]}</span>`,
                );
            }
        }
...
declare global {
    // trick to augment the global TagName with this component
